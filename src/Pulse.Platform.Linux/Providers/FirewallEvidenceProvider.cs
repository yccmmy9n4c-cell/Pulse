...
{
    public const string InactiveSummary = "Pulse did not find an active UFW, firewalld, or nftables systemd service.";
    ...
    public const string InactiveSource = "systemctl is-active ufw.service; systemctl is-active firewalld.service; systemctl is-active nftables.service";
    ...
    public async Task<EvidenceResult> CollectAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (!ufw.Started && !firewalld.Started && !nftables.Started)
        {
