...
public sealed class DistributionCompatibilityEvidenceProvider : ILinuxEvidenceProvider
{
    ...
    public Task<EvidenceResult> CollectAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            DistributionSupportLevel.Supported => $"{support.DisplayName} is inside the declared Pulse FE support boundary.",
            DistributionSupportLevel.UnverifiedDerivative => $"{support.DisplayName} reports Fedora compatibility but is not yet verified by Pulse.",
    ...
    public Task<EvidenceResult> CollectAsync(CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(current switch
        {
            "x64" => new EvidenceResult(Id, "Architecture compatibility", EvidenceState.Healthy,
                "This system is linux-x64, the primary Pulse FE release-candidate architecture.",
                "Use the architecture-specific x86_64 RPM package. Pulse never substitutes a package built for another architecture.",
            ...
            "arm64" => new EvidenceResult(Id, "Architecture compatibility", EvidenceState.Informational,
                ...
                "Use only a verified aarch64 RPM package when it becomes available; do not install the x86_64 package on this system.",
                "RuntimeInformation.ProcessArchitecture"),
    ...
    public Task<EvidenceResult> CollectAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(desktop))
        {
            return Task.FromResult(new EvidenceResult(Id, "Desktop environment", EvidenceState.Informational,
                ...
                "This is normal for headless scheduled assessments. In an interactive session, Pulse supports conventional Fedora desktops and falls back to in-app guidance when a desktop utility is unavailable.",
...
{
    private static readonly string[] CoreTools = ["systemctl", "journalctl", "rpm", "dnf", "ip", "findmnt", "lsblk"];
    private static readonly string[] OptionalTools = ["nmcli", "smartctl", "firewall-cmd", "nft", "systemd-analyze", "getenforce"];
