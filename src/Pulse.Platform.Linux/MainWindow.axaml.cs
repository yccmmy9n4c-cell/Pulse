...
public sealed partial class MainWindow : Window
{
    ...
    private void RenderSupportBoundary()
    {
        ...
        if (!supported)
        {
            ...
            UpdateDetailText.Text = $"Pulse FE updates are restricted to verified Fedora desktops. {_support.Message}";
    ...
    private void RenderDashboardDomains(IReadOnlyList<EvidenceResult> results)
    {
        ...
        ApplyDomain(results, ["linux.rpm-verifydb", "linux.rpm-inventory", "linux.dnf-cached-updates", "linux.dnf-security-updates", "linux.dnf-automatic", "linux.reboot-required"],
        ...
        ApplyDomain(results, ["linux.selinux", "linux.firewall-indicator", "linux.dnf-security-updates", "linux.dnf-automatic", "linux.luks-indicator", "linux.secure-boot"],
    ...
    private void RenderPackageIntelligence(IReadOnlyList<EvidenceResult> results)
    {
        ...
        {
            FindEvidence(results, "linux.rpm-verifydb"),
            FindEvidence(results, "linux.rpm-inventory"),
            FindEvidence(results, "linux.dnf-cached-updates"),
            FindEvidence(results, "linux.dnf-security-updates"),
            FindEvidence(results, "linux.dnf-automatic"),
        ...
        if (available.Length == 0)
        {
            ...
            PackageExecutiveDetailText.Text = "Run an assessment to evaluate local RPM/DNF package state without refreshing repositories or installing updates.";
    ...
    private void RenderSecurityIntelligence(IReadOnlyList<EvidenceResult> results)
    {
        ...
        {
            FindEvidence(results, "linux.selinux"),
            FindEvidence(results, "linux.firewall-indicator"),
            FindEvidence(results, "linux.dnf-security-updates"),
            FindEvidence(results, "linux.dnf-automatic"),
            FindEvidence(results, "linux.luks-indicator"),
    ...
    private static void ConfigureReviewAction(Button button, EvidenceResult? evidence)
    {
        ...
        {
            "linux.dnf-cached-updates" or "linux.dnf-security-updates" or "linux.dnf-automatic" => "Open Software Updater",
    ...
    private void ReviewActionButton_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        if (evidence.ProviderId is "linux.dnf-cached-updates" or "linux.dnf-security-updates" or "linux.dnf-automatic" &&
            TryLaunchInstalledTool(["gnome-software", "dnfdragora", "plasma-discover"], "software updater"))
        {
            return;
        }
        ...
        if (evidence.ProviderId == "linux.firewall-indicator" &&
            TryLaunchInstalledTool(["firewall-config"], "firewall settings"))
        {
            return;
        }
    ...
    private async void CheckForUpdatesButton_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        UpdatePackageText.Text = "Selecting a compatible Fedora RPM package and checksum file.";
    ...
    private async void DownloadUpdateButton_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        UpdateDownloadStatusText.Text = "Downloading the Fedora RPM package and preparing SHA-256 verification…";
