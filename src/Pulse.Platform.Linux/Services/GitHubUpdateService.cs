...
public sealed class GitHubUpdateService
{
    ...
    public static PulseUpdateResult EvaluateReleaseList(string json, string currentVersion, string packageArchitecture)
    {
        ...
        {
            return new(UpdateAvailability.Unavailable, currentVersion, null,
                $"No published Pulse Linux AE pacman package for {packageArchitecture} was found. The repository may not have a compatible release yet.",
    ...
    private static ReleaseCandidate? CreateCandidate(GitHubRelease release, string architecture)
    {
        ...
        var expectedPackage = $"pulse-platform-{version.ToString(4)}-1-{architecture}.pkg.tar.zst";
